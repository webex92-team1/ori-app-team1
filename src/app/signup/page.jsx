...
import { ChefHat, User, Mail, Lock, Loader2 } from "lucide-react"; // Loader2 (Spinner) をインポート
...
export default function SignupPage() {
  ...
  const getErrorMessage = (errorCode) => {
    switch (errorCode) {
      ...
      case "auth/operation-not-allowed":
        return "開発者にお問い合わせください（メール/パスワード認証が無効です）。";
      default:
  ...
  const handleGoogleSignup = async () => {
    ...
    try {
      ...
      // GoogleログインでもFirestoreにプロファイルを作成（既存の場合はマージまたはスキップされる想定）
      await createUserProfile({
        uid: user.uid,
        email: user.email,
