...
| 認証           | **Firebase Auth（Google ログイン）** | ログインの導入が最も簡単                    |
...
| **Firebase Auth**      | Google ログインが即実装可能（コード数行）              |
...
/src
 ├─ app
 │   ├─ page.js                // トップページ
 │   ├─ ingredients/page.js    // 食材入力
 │   ├─ recipes/page.js        // レシピ一覧
 │   ├─ recipe/[id]/page.js    // レシピ詳細
 │   ├─ mypage/page.js         // マイページ
 │   └─ login/page.js          // ログイン
 ├─ components
 │   ├─ RecipeCard.js
 │   ├─ Header.js
...
- クライアント → API を直接叩く
- URL に入れるのは「食材の文字列」だけ
- マッチング方法：**完全一致のみ**（シンプルで実装しやすい）
- 足りない食材は自前ロジックで灰色表示する
